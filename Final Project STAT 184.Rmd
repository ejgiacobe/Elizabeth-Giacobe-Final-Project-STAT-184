...
Transport <-
  RawTransport %>%
  select(COUNTRY, AMOUNT, DATE) %>%
  rename(country = COUNTRY, distance = AMOUNT, year = DATE) %>%
...
Countries <-
  RawCountries %>%
  select(COUNTRY, AMOUNT, DATE) %>%
  rename(country = COUNTRY, pop_dens = AMOUNT, year = DATE) %>%
...
```{r}
CountriesTransportPop <-
  inner_join(Countries, Transport, by = c("country", "year"))

CountriesTransportPop
```
```{r}
MeaslesPop <-
  left_join(Countries, Measles, by = c("country", "year"))

MeaslesPop

MeaslesPopTransport <-
  left_join(Transport, MeaslesPop, by = c("country", "year"))
