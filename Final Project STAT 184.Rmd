...
mod1 <-
  MeaslesPopTransport %>%
  mutate(vac_tot = vac1 + vac2) %>%
  rpart(outbreaks ~ pop_dens + distance + vac_tot, data = .)
...
LatLng <-
  RawLatLon %>%
  rename("country" = "X1", "capital" = "X2", "lat" = "X3", "lng" = "X4") %>%
...
```{r}
MeaslesFull <-
  left_join(MeaslesPopTransport, LatLng, by = "country", "country")
```
...
MeaslesMap <-
  leaflet(MeaslesFull) %>%   # like ggplot()
  addTiles() %>%          # add the map
  addCircleMarkers(radius=0.01, color="red") %>%
  addCircles(radius = ~500 , color = "blue", opacity = 0.0001)
