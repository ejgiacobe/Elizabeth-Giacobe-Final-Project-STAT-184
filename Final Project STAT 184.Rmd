...
```{r, message=FALSE}
# getting data
MeaslesOutbreaksRaw <- read_csv("C:\\Users\\Lizzie\\Downloads\\MeaslesOutbreaks.csv")

MeaslesVac1Raw <- read_csv("C:\\Users\\Lizzie\\Downloads\\Measles1Vac.csv")

MeaslesVac2Raw <- read_csv("C:\\Users\\Lizzie\\Downloads\\Measles2Vac.csv")
...
```{r}
mod1 <-
  lm(outbreaks ~ vac1 + vac2 + pop_dens + year + population + distance + lat + lon, data = MeaslesFull)
summary(mod1)
```
...
MapReadyMeasles <-
  MeaslesFull %>%
  group_by(lat, lon) %>%
  summarise(tot_outbreaks = sum(outbreaks))

MeaslesMap <-
  leaflet(MapReadyMeasles) %>%   # like ggplot()
  addTiles() %>%          # add the map
  addCircleMarkers(radius=0.01, color="red") %>%
  addCircles(radius = ~(tot_outbreaks) , color = "blue", opacity = 0.0001)
